...
const CSUtil = require('@containership/containership.utils');
            ...
            function safeDeleteRC(delete_cb) {
                self.getApplication(name, (err, application) => {
                    ...
                    return async.parallel([
                        ...
                        // need to return callback manually so results from parallel aren't returned
                        // need value to be empty so api returns 204 on success
                        return delete_cb(err);
                    });
                });
            }
