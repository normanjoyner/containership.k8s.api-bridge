...
class KubernetesApi extends ApiImplementation {
    ...
    static k8sServiceDescriptionFor(appDesc, ports, discoveryPorts) {
        ...
        return {
            kind: 'Service',
            metadata: {
                name: `${_.join(_.take(appName, 15), "")}-service` //Limit to 24 chars
            },
            spec: {
                type: 'NodePort',
    ...
    static k8sToCSApp(k8sApp) {
        return _.merge({
            ...
            _.reduce(KubernetesApi.CS_TO_K8S_APPLICATION_MAPPING, (csApp, k8sPaths, csKey) => {
                ...
                const v = conversionFn && baseValues ? conversionFn.apply(null, baseValues) : _.first(baseValues);  //Retrofitted to use all the vals.
            ...
            return v ? 
                _.merge(k8sApp, 
                    _.reduce(k8sPaths, (setK8SPaths, path) => {
                        ...
                        console.log("Setting fullpath: " + JSON.stringify(fullPath));
    ...
    enforceConstraints(hosts, app) {
        const perHost = parseInt(_.get(app, ["tags", "constraints", "per_host"], "0"));

        const followers = _.filter(_.values(hosts), (h) => h.mode === 'follower');
        const followerCount = _.size(followers);
        const neededReplicas = followerCount * perHost;

        console.log("Enforcing constraints, need " + neededReplicas + " for app " + app);

        this.updateApplication(app.id, {count: neededReplicas}, 
    ...
    updateApplication(appId, csAppDesc, cb) {
        ...
        const k8sDesc = KubernetesApi.csToK8SApp(csAppDesc);

        console.log("Patching with " + JSON.stringify(k8sDesc));

        //Look here 
        //Possible need to pass full app desc to this endpoint.
        ...
        request({ 
            ...
            console.log("AFTER PATCH: " + err + " : " + JSON.stringify(body));
    ...
    createApplication(csAppDesc, cb) {
        ...
        console.log("Posting replication controller: " + JSON.stringify(k8sAppDesc));
        ...
        request({ 
            ...
            if(!err) {
                ...
                console.log("Back with rep controller: " + JSON.stringify(body));
...
KubernetesApi.CS_TO_K8S_CONVERSIONS = {
    ...
    'volumes': (volumes, destinationPath) => {
        return _.includes(destinationPath, 'containers') ?
            _.map(volumes, (v) => ({
                name: _.replace(v.host, new RegExp("/", 'g'), ""),
...
add(['volumes'], (volumeMounts, volumeDests) => {
