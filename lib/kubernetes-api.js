...
class KubernetesApi extends ApiImplementation {
    ...
    static k8sServiceDescriptionFor(app_desc, ports, discovery_ports) {
        ...
        return {
            kind: 'Service',
            metadata: {
                name: `ser${_.join(_.take(String(encode(app_name)), 19), '')}` // Limit to 24 chars
            },
            spec: {
                type: 'NodePort',
    ...
    getContainers(cb) {
        return request(`${this.k8s_api_url}/api/v1/pods`,
            (err, res, body) => {
                if(err || res.statusCode !== 200) {
    ...
    updateApplication(app_id, app_desc_update, cb) {
        return this.getApplication(app_id, (err, existing_app) => {
            ...
            const updateReplicationController = (cb) => {
                ...
                this._applyConstraintsToApplication(existing_app, (err, app) => {
                    if(err) {
                        return cb(err);
                    }

                    if(!replication_controller_needs_updated && _.isEqual(existing_app, app)) {
                        return cb();
                    } else {
                        existing_app = app;
                    }

                    const k8sDesc = Translator.csApplicationToK8SReplicationController(existing_app);
    ...
    deleteHost(host_id, cb) {
        return request({
            ...
            // immediately trigger constraint update since host count has changed
