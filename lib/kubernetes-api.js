...
const Translator = require('@jeremykross/containership.k8s.translator');
...
class KubernetesApi extends ApiImplementation {
            ...
            request(`${this.k8sApiUrl}/api/v1/namespaces/default/services`, util.ifSuccessfulResponse((services) => {
                this.getHosts((hosts) => {
                    ...
                    const csItems = _.map(k8sItems, _.flow(
                        Translation.csApplicationFromK8SReplicationController,
                        (csApp) => {
    ...
    updateApplication(appId, csAppDesc, cb) {
        this.getApplications((apps) => {
            ...
            const k8sDesc = Translation.csApplicationToK8SReplicationController(_.merge(existingApp, newApp));
    ...
    createApplication(csAppDesc, cb) {
        ...
        const k8sAppDesc = Translation.csApplicationToK8SReplicationController(_.merge(csAppDesc, {
            host_port: hostPort, 
                ...
                const containersByNode = _.flow(
                    _.partial(_.map, _, (pod) => { 
                        ...
                        const csContainer = _.merge(
                            Translation.csApplicationFromK8SPodSpec(k8sContainer),
                            { 
